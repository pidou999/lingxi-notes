...
export async function POST(request: NextRequest) {
  try {
    ...
    if (protocol === "Anthropic") {
      ...
      // OpenAI 兼容协议：baseUrl 为用户配置的 AI 服务地址。
      // SSRF 防护：仅拦截本机 loopback / 云元数据，允许局域网模型服务（如 192.168.x.x 的 ollama）。
      // 注意：不再对 SSL 错误自动降级 rejectUnauthorized，避免中间人劫持。
      try {
    ...
    console.error("[proxy/models]", err?.message || err, err?.stack || "");
    return NextResponse.json(
