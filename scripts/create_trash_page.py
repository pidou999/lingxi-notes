...
import { Trash2, RefreshCw, RotateCcw, AlertTriangle } from "@ai-notes/icons";
    ...
    <div className="mx-auto max-w-4xl">
      ...
      {notes.length === 0 ? (
        ...
        <div className="space-y-2">
          {notes.map((note) => {
            const days = note.deletedAt
              ? Math.floor((Date.now() - new Date(note.deletedAt).getTime()) / 86400000)
              : 0;
            return (
              <div
                key={note.id}
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <Button variant="ghost" size="sm" onClick={() => handleRestore(note.id)} className="text-brand-600 hover:bg-brand-50 dark:text-brand-400 dark:hover:bg-brand-900/20" aria-label="恢复笔记">
                    <RotateCcw size={16} />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handlePermanentDelete(note.id)} className="text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20" aria-label="永久删除">
                    <Trash2 size={16} />
                  </Button>
                </div>
              </div>
